...
sbit R_LED=P3^5;
sbit L_LED=P3^4;
sbit R_LED_DOWN=P3^2;
sbit L_LED_DOWN=P3^3;



//循迹避障模块接线
sbit left_led1 = P3^6;	//P3^3;//左寻迹信号 为0 没有识别到黑线 为1识别到黑线
sbit right_led1= P3^7;	//P3^2;//右寻迹信号	为0 没有识别到黑线 为1识别到黑线
...
void find_track()
{
	if(left_led1 == 0 && right_led1 == 0)//左右寻迹探头识别到黑线
	{
		accelerate();
		accelerate();
		go_forward();//前进
	...
	{
		if(left_led1 == 1 && right_led1 == 0)//小车左边出线，右转修正
		{
			go_stop();
			delay(20,0);
			go_left();//左转
		...
		if(left_led1 == 0 && right_led1 == 1)//小车右边出线，左转修正
		{
			go_stop();
			delay(20,0);
			go_right();//右转
		...
		/*if(left_led1 == 1 && right_led1 == 1)//左右寻迹探头都没识别到黑线
		{
			go_back();//后退
			delay(20,0);
			go_stop();
